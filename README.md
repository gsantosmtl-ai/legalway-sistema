...
`docs/assets/i18n.js`: dicionário `frase em português → [es, en]` aplicado depois que a tela carrega, mais um MutationObserver com fila (traduz o que o JS renderiza depois). Idioma vem de `?lang=`, de `localStorage['legalway-idioma']` ou do navegador. Seletor no menu do usuário (comum.js) e no topo do portal do cliente. Traduzidos hoje: portal do cliente inteiro, login/primeiro acesso, menu lateral, títulos e rótulos comuns — o resto continua em português até entrar no dicionário. `LW.traduzir(txt)` traduz uma frase no próprio JS (usado no portal).
